...
def main_demo(basic_demo_dict,show_detailed = False):
    ...
    physics_predictions_test = basic_demo_dict['physics_predictions']
    physics_mse = basic_demo_dict['physics_mse']
    physics_r2 = basic_demo_dict['physics_r2']
    physics_residuals = basic_demo_dict['physics_residuals']

    # Train on full training set
    data_predictions_test = basic_demo_dict['data_predictions']
    data_mse = basic_demo_dict['data_mse']
    data_r2 = basic_demo_dict['data_r2']
    data_residuals = basic_demo_dict['data_residuals']

    learned_slope = basic_demo_dict['data_slope']
    learned_intercept = basic_demo_dict['data_intercept']
    TRUE_PHYSICS_SLOPE = basic_demo_dict['physics_slope']
    physics_weighted_ad = basic_demo_dict['physics_weighted_anomalies']
    data_weighted_ad = basic_demo_dict['data_weighted_anomalies']
    ...
    **Why This Matters**: In network monitoring, we need to detect when latency is unusually high, 
    ...
    physics_ad = basic_demo_dict['physics_anomalies']
    data_ad = basic_demo_dict['data_anomalies']
    ...
    with col1:
        ...
        with col1d:
            st.metric("Weighted F1-Score", f"{physics_weighted_ad['f1']:.3f}")
    ...
    with col2:
        ...
        with col2d:
            st.metric("Weighted F1-Score", f"{data_weighted_ad['f1']:.3f}")
        st.write(f"Detected: {data_ad['n_detected']} anomalies")

    st.markdown("## Uncertainty Aware Anomaly Detection Performance")

    #st.write(f"{'Method':<20} {'Standard F1':<12} {'Weighted F1':<12} {'Coverage':<10}")
    #st.write("-" * 55)
    #st.write(f"{'Physics':<20} {physics_ad['f1']:<12.3f} {physics_weighted_ad['f1']:<12.3f} ")
    #st.write(f"{'Data-Driven':<20} {data_ad['f1']:<12.3f} {data_weighted_ad['f1']:<12.3f} ")
